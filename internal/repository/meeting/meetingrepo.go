...
func (m *MeetingRepo) GetMeetingsByExpertId(expertId int) ([]model.Meeting, error) {
	query := `SELECT id, user_id, expert_id, time_start, time_end, total_cost, meeting_link, meeting_id, status FROM meeting_transactions WHERE expert_id=$1 AND time_start > NOW() + INTERVAL '5 hours'`
...
func (m *MeetingRepo) GetMeetingsByUserId(userId int) ([]model.Meeting, error) {
	query := `SELECT id, user_id, expert_id, time_start, time_end, total_cost, meeting_link, meeting_id, status FROM meeting_transactions WHERE user_id=$1 AND time_start > NOW() + INTERVAL '5 hours'`
...
func (m *MeetingRepo) GetExpertAvailableMeets(expertId int) ([]model.Meeting, error) {
	query := `SELECT id, user_id, expert_id, time_start, time_end, total_cost, meeting_link, meeting_id, status 
              ...
              WHERE expert_id=$1 AND status='available' AND time_start > NOW() + INTERVAL '5 hours'`
